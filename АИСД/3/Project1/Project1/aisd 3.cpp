...
void quickSort(int* ptr, int first, int last) {     //функция сортировки
	int mid, tmp;
	int f = first, l = last;
	mid = ptr[(f + l) / 2]; 				//вычисление опорного элемента
	do
	{
		while (ptr[f] < mid) f++;
		while (ptr[l] > mid) l--;
	...
	if (first < l) quickSort(ptr, first, l);
	if (f < last) quickSort(ptr, f, last);
}
